...
class CostController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Cost']))
		{
			...
			if($model->save()){
				if($model->status==1){
					AccountCMP::decreaseBalance($model->user_id, $model->amount);
					AccountCMP::newDebtor($model->amount,$model->id,null, $model->user_id, $model->costType->name);
				}				
		...
		if(isset($_POST['Cost']))
		{
			...
			if($model->save()){
				if($model->status==1){
					...
					$acountItem = AccountCMP::findDebtor($model->id);
					if(isset($acountItem)){								
						AccountCMP::increaseBalance(Yii::app()->user->id, $previous_amount);
						AccountCMP::decreaseBalance(Yii::app()->user->id, $model->amount);
						AccountCMP::updateDebtor($model->id, $model->amount, $model->costType->name);
					}else{
						AccountCMP::decreaseBalance($model->user_id, $model->amount);
						AccountCMP::newDebtor($model->amount,$model->id,null, $model->user_id, $model->costType->name);												
					}
					...
					AccountCMP::dellDebtor($model->id);
				}				
				$this->redirect(array('view','id'=>$model->id));
			}
		}
	...
	{
		$dataProvider=new CActiveDataProvider('Cost');
