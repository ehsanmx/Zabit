...
class Income extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('amount, user_id, income_type_id', 'required'),
			array('amount, status, income_type_id', 'numerical', 'integerOnly'=>true),
			array('user_id', 'length', 'max'=>20),
			array('description, create_date', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, amount, status, description, create_date, user_id, income_type_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('income_type_id',$this->income_type_id);
